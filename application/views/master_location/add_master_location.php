    ...
    <div class="container-fluid">
      ...
      <!-- <div class="row"> -->
        ...
        <form class="" action="<?= base_url('master_location/proses_add_master_location'); ?>" method="post">
         <div class="form-group row">
        <label for="inputBin" class="col-sm-2 col-form-label">Bin</label>
        <div class="col-sm-10">
          <input type="text" class="form-control" id="inputBin" name="bin" >
        </div>
      </div>
      <div class="form-group row">
        <label for="inputStorage_Location" class="col-sm-2 col-form-label">Stotrage Location</label>
        <div class="col-sm-10">
          <input type="text" class="form-control"  id="inputStorage_Location" name="storage_location" >
        </div>
      </div>
      <div class="form-group row">
...
<div class="modal fade" id="exampleModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">code Location</h5>
        <button type="button" id="btn2" class="close" data-dismiss="modal2" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <table class="table table-hovered">
          ...
          <tbody>
            <?php foreach ($all as $key => $value) { ?>
              <tr onclick="getData(<?= $key; ?>)" style="cursor: pointer">
                <th scope="row"><?= $key+1; ?>.</th>
                <td id="code_location<?= $key; ?>" value="<?= $value['code_location']; ?>"><?= $value['code_location']; ?></td>
              </tr>
            <?php } ?>
          </tbody>
        </table>
...
function getData(row)
{
  var code_location = document.getElementById("code_location"+row).innerHTML;

  document.getElementById("inputCode_Location").value = code_location;
 

  $('#exampleModal2').modal2('hide');
}
</script>